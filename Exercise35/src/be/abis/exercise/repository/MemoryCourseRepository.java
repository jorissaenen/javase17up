...
public class MemoryCourseRepository implements CourseRepository{
    ...
    private static MemoryCourseRepository instance;
    private List<Course> courses = new ArrayList<>();

    private MemoryCourseRepository(){
    ...
    public static MemoryCourseRepository getInstance(){
        if(instance==null)instance=new MemoryCourseRepository();
        return instance;
    }
    ...
    @Override
    public List<Course> findCoursesOfDuration(int numberOfDays) {
        return courses.stream().filter(c->c.days()==numberOfDays).toList();
    }
}
